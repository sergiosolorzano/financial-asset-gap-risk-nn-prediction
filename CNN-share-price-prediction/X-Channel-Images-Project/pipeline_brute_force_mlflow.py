...
def mlflow_log_params(iteration, experiment_name, experiment_id):
    ...
    params_dict = {
        ...
        "training_cols_used": Parameters.training_cols_used,
...
def brute_force_function(credentials):
    ...
    if experiment is not None:
        ...
        mlflow.create_experiment(Parameters.mlflow_experiment_name, artifact_location="wasbs://mlflow@artstoragepublic.blob.core.windows.net/mlartifacts")#wasbs://<container>@<server>.blob.core.windows.net/<dir>
        experiment = mlflow.get_experiment_by_name(experiment_name)
        experiment_id = experiment.experiment_id
    ...
    for sc in scalers:
        for s in sample_ranges:
            ...
            with mlflow.start_run(run_name=f"run_{iteration}") as run:
